...
export const UpdateAvatar = async (req, res) => {
  try {
    ...
    // Check if user has an existing image that isn't the default "example" URL
    if (user.image !== "example") {
      const oldKey = user.image.split("/").pop(); // Extract key from URL

      // Delete the old image from S3/DO Spaces
      await s3
        .deleteObject({
    ...
    const uploadParams = {
      Bucket: process.env.DO_SPACE_NAME,
      Key: `images/${Date.now()}_${file.originalname.replace(/\s+/g, "_")}`, // Avoid spaces
      Body: file.buffer,
      ACL: "public-read",
    ...
    res
      .status(200)
      .json({ message: "Upload Success", imageUrl: data.Location });
  } catch (error) {
